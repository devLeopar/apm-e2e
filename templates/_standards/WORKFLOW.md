...
**Optional validation and integration layers** - Projects that require end-to-end behavior verification opt into an E2E Validation Loop (§7.4) where the Worker executes behavioral scenarios inline during Task Validation, iterating on test-side issues within a per-scenario budget and re-running scenarios after application-side fixes. Projects that depend on external services or tooling declare MCP Dependencies (§7.5) that the Manager coordinates setup for during initiation. Both layers are policy-driven and inert when the project does not need them - their presence is determined by what the Planner captures during Context Gathering.
...
When the project declares end-to-end behavior verification or external service integrations, the Spec includes an **E2E Testing Policy** section capturing the project's validation policy (auto, ask-per-task, or never) and an **MCP Dependencies** section listing external MCPs required for execution alongside their purposes and authentication requirements. See §7.4 and §7.5.
...
**E2E Validation integration** - When the project's E2E Testing Policy (§7.4) is not `never`, the Manager renders the Task's Plan Validation acceptance criteria into test-executable scenarios and includes them as an E2E Validation section in the Task Prompt. Under `ask-per-task`, the Manager prompts the User before dispatch with a recommendation based on the Task's nature (UI-heavy Tasks warrant E2E; pure refactors typically do not). Under `auto`, scenarios are always rendered unless the Task has no observable user-facing behavior. Under `never`, no E2E section is added.
...
**E2E Validation execution** - When the Task Prompt contains an E2E Validation section, the Worker assembles an E2E Test Brief by enriching the Manager-rendered scenarios with runtime environment (target URL or app identifier, launch commands, available MCPs, artifact path) and executes the scenarios inline per §7.4. Within each scenario, the Worker iterates on test-side issues (selector adjustments, timing, retries) up to the iteration budget without mutating the application. On a scenario's application-side failure, the Worker exits to the standard Correction Loop, applies a targeted code fix, then re-runs all scenarios from the start. When the User issues a correction that changes observable behavior and the project's E2E Policy is not `never`, the Worker re-runs the scenarios after applying the correction - under `ask-per-task`, the Worker first asks the User whether re-validation is wanted. Post-Done User corrections within the original Task's scope are handled by the Worker with re-validation and a Post-Done section appended to the Task Log; scope-external corrections are directed back through the Manager for new Task creation.
...
**E2E result interpretation** - When a Task Log contains an E2E Validation section, the Manager reads the structured `e2e_result` alongside Summary, Details, and Validation. A passing `e2e_result` with `status: Success` proceeds normally. A failing `e2e_result` with `status: Success` is a hallucination signal - investigate before accepting. A `blocked` `e2e_result` (missing MCP or environment issue) triggers §7.5 setup re-guidance before re-dispatch. A failing `e2e_result` with `status: Partial` or `Failed` generates a follow-up Task Prompt refined from the `failure_summary` and failing scenario details.
...
### 7.4 E2E Validation Loop

**Runtime:** `{SKILL_PATH:apm-e2e-validation}`

**Policy.** The project's E2E Testing Policy is declared by the Planner in the Spec's E2E Testing Policy section. Three values are defined: `auto` (E2E runs by default on every Task with observable user-facing behavior), `ask-per-task` (the Manager asks the User before each dispatch, presenting a recommendation based on Task nature), and `never` (E2E is not part of the workflow). Policy is inherited by every Task unless the Task's Validation criteria explicitly disable it. Under `ask-per-task`, the Manager's recommendation considers Task nature, project risk, and accumulated observations - a UI-heavy Task with behavioral acceptance criteria warrants E2E; a pure refactor typically does not.

**Scenario Rendering.** E2E content traverses three rendering levels. The Planner writes product-level acceptance criteria in the Task's Plan Validation field during Work Breakdown ("User can log in with valid credentials and reach the dashboard") - these remain implementation-agnostic and tool-agnostic. The Manager, during Task Prompt Construction, renders these criteria into test-executable scenarios with observable user actions and pass conditions, placing them in the Task Prompt's E2E Validation section. The Worker, at execution time, enriches scenarios with runtime environment (target URL or app identifier, launch commands, available MCPs, artifact path) into a structured E2E Test Brief that drives execution.

**Inline Execution.** The Worker executes the E2E Test Brief inline during Task Validation - selecting a runner per `{SKILL_PATH:apm-e2e-validation}` §5.1 (Playwright MCP for web, Maestro for Expo, etc.), running each scenario in order, and capturing a structured `e2e_result` (status, per-scenario outcomes, artifact paths, failure summary) directly into the Task Log. Execution stays in the Worker's context window - test runner output, screenshots, and retry traces accumulate alongside other Task work. This trades Worker context budget for fewer moving parts: no separate dispatch, no inter-agent contract to maintain, and the same agent that writes the application code runs the behavioral checks against it.

**Iteration.** Within each scenario, the Worker iterates on test-side issues (selector adjustments, timing, retries) up to the iteration budget declared in the Brief (default 3 attempts per scenario). Test-side adjustments do not change application code. When a scenario fails for application-side reasons (actual behavior mismatch), the Worker stops the per-scenario retry loop, exits to the standard Correction Loop in §7.2, applies a targeted code fix, then re-runs all scenarios from the start. Application-side iterations are bounded by the Worker's standard Correction Loop budget - exceeding it ends the Task with Partial status.

**User corrections.** User corrections that change observable behavior trigger a re-run of the scenarios when the E2E Policy is not `never`. Under `auto`, re-running is the default; the User can opt out explicitly. Under `ask-per-task`, the Worker asks the User whether to re-validate before re-running. Scope-internal Post-Done corrections are handled by the Worker with re-validation and a Post-Done Correction section appended to the Task Log; scope-external corrections are directed back through the Manager for new Task creation per §3.4 Document Modification.

**Failure Handling.** If execution returns `blocked` (missing MCP, unreachable environment, no available runner), the Worker pauses execution, records the block in the Task Log, and reports Partial status with the missing dependency. The Manager re-presents setup guidance per §7.5 and re-dispatches after User confirmation. If execution returns `fail` after exhausting both per-scenario test-side retries and the Correction Loop's application-side iterations, the Task ends with Partial or Failed status per outcome standards; the Manager reviews the `failure_summary` and issues a refined follow-up Task Prompt targeting the observed failure.

**Test Code Persistence and Regression Suite.** Projects that opt in declare an `## E2E Test Corpus` section in the Spec - corpus path in the project tree (e.g., `.maestro/`, `e2e/playwright/`), runner formats, and regression cadence (`per-task`, `per-stage`, or `manual-only`). After a Worker's E2E passes, the Worker writes the passing scenarios as runner-native test code into the corpus (Maestro YAML, Playwright spec) with a `# APM source: Task N.M` header for traceability. The Manager runs the full corpus regression at the cadence declared - by default after each E2E-passing Task during Task Review per `{GUIDE_PATH:task-review}` - and triages regression failures into three branches: consumer broke producer's feature (follow-up to current Worker), producer's test is stale (follow-up to producer Worker or Manager updates inline), or consumer adapts to existing contract (follow-up to current Worker with adaptation guidance). Ambiguous triage routes to the User. Empty corpus (first E2E-passing Task) makes regression a no-op; corpus runs are bounded by the standard Correction Loop budget when iterations loop back through Task Execution.

### 7.5 MCP Coordination

**Runtime:** `{SKILL_PATH:apm-mcp-setup}`

**Declaration.** The Planner captures external service requirements during Context Gathering (Round 2 Technical Requirements focus area) and records them in the Spec's MCP Dependencies section during Work Breakdown. Each entry lists the MCP name, its purpose (which Stages or Tasks need it), installation command, and authentication requirements. The MCP registry shipped with `{SKILL_PATH:apm-mcp-setup}` provides canonical installation commands and authentication flows for known MCPs; when a project needs an MCP not in the registry, the Planner asks the User for installation details.

**Manager-owned setup.** The Manager reads the Spec's MCP Dependencies during First Initiation and presents setup guidance to the User alongside Version Control conventions, before the first dispatch. For CLI-only MCPs, the Manager provides the exact install command. For MCPs requiring authentication, the Manager directs the User through the dashboard → credential extraction → MCP install sequence captured in the registry. The Manager pauses and waits for User confirmation that each MCP is installed and authenticated before proceeding. Workers use MCPs that are already installed - they do not install MCPs themselves.

**Runtime missing-MCP handling.** When Task Execution encounters a missing MCP (E2E execution returns `blocked`, or the Worker detects the MCP is not available), the Worker reports Partial status with the missing MCP listed in the Task Log's Issues section. The Manager re-presents setup guidance to the User per Manager-owned setup above and re-dispatches after confirmation. Scope stays clean: Workers never install MCPs; the Manager always re-presents guidance; the User performs the installation.
