...
1. Read the following documents (these reads are independent):
   ...
   - `{SKILL_PATH:apm-e2e-validation}` - E2E policy, scenario rendering, and inline execution standards
   - `{SKILL_PATH:apm-mcp-setup}` - MCP setup coordination and registry
...
3. Present understanding summary and VC conventions together for User approval, covering:
   ...
   - *E2E policy, corpus, and MCP preview:* when the Spec declares an `## E2E Testing Policy`, state the policy value (`auto`, `ask-per-task`, or `never`) and how it will apply across the project. When the Spec declares `## E2E Test Corpus`, state the corpus path, runner formats, and regression cadence (`per-task`, `per-stage`, or `manual-only`) - this is where Workers will persist passing tests and where you will run regression per `{SKILL_PATH:apm-e2e-validation}` §7. When the Spec declares `## MCP Dependencies`, list the required MCPs with their purposes - detailed setup guidance with install commands and authentication flows follows after approval, before first dispatch. Omit this bullet when none of these sections are present.
4. Ask the User to review the understanding summary, the proposed conventions, and the E2E/MCP preview (when applicable) and confirm before proceeding.
   - If corrections needed, integrate feedback and re-present.
   - If approved, write the Tracker's Version Control table (one row per repository with base branch, branch convention, and commit convention), write commit conventions to `{RULES_FILE}` within the APM_RULES block, populate Task Tracking with Stage 1 Tasks per `{GUIDE_PATH:task-review}` §4.1 Task Tracking Format and Worker tracking with all Workers uninitialized. When the Spec's MCP Dependencies section lists required MCPs, present setup guidance per `{SKILL_PATH:apm-mcp-setup}` §3 Manager-Owned Setup Flow and pause for User confirmation that each MCP is installed and authenticated before dispatching. Then generate the first Task Prompt(s) per `{GUIDE_PATH:task-assignment}` §3.1 Dispatch Assessment and proceed to §3 Continuous Coordination.
