...
**Validation criteria:** Each Task specifies concrete pass/fail criteria tied to its deliverables - what to check and how. Criteria the Worker can verify autonomously (tests pass, outputs exist with correct structure, behavior matches requirements) allow autonomous iteration. Criteria requiring User involvement - judgment (design approval, content quality) or action (running external checks, confirming platform behavior) - require the Worker to pause. Most Tasks combine multiple criteria. Validation criteria are Worker-scoped - no references to other Stages, Tasks, or coordination-level gates. Workers validate their own deliverables; holistic verification beyond individual Tasks - milestone checks, acceptance gates, end-to-end validation at a Stage level - is the Manager's runtime responsibility. When Context Gathering surfaced observations about holistic verification needs, include them in Plan notes per §2.1 Workflow Context - the Manager acts on them at runtime, not as planned Tasks. When the project's E2E Testing Policy is not `never` and the Task has observable user-facing behavior, phrase at least one validation criterion as a product-level acceptance statement (user action and expected outcome in natural prose) - the Manager renders this into test-executable scenarios at runtime per `{SKILL_PATH:apm-e2e-validation}` §3 Three-Level Scenario Rendering. Keep Plan-level acceptance implementation-agnostic: no framework names, selectors, or DOM paths.
...
**E2E and MCP sections:** When Context Gathering captured an end-to-end validation preference, the Spec includes an `## E2E Testing Policy` section with the chosen policy value (`auto`, `ask-per-task`, or `never`) and brief rationale per `{SKILL_PATH:apm-e2e-validation}` §2 E2E Testing Policy. When the policy is not `never`, the Spec also includes an `## E2E Test Corpus` section declaring the corpus path in the project tree, runner formats, and regression cadence per `{SKILL_PATH:apm-e2e-validation}` §6.2 Storage Convention and §7.1 Trigger and Cadence - this drives test code persistence and Manager-run regression. When the project depends on external services with MCP integrations, the Spec includes an `## MCP Dependencies` table per `{SKILL_PATH:apm-mcp-setup}` §2 Declaration. All sections are omitted when not applicable - absent declaration is equivalent to `never` and no dependencies.
...
1. Analyze design decisions from gathered context:
   ...
   - *E2E Testing Policy.* When Context Gathering captured an E2E preference, record the chosen policy (`auto`, `ask-per-task`, or `never`) and brief rationale. When no preference was captured and the project has user-facing behavior, surface the question to the User now and wait for a decision before writing. When the project has no user-facing behavior, `never` applies implicitly and no section is written.
   - *E2E Test Corpus.* When E2E policy is not `never`, declare the corpus path in the project tree (defaults per `{SKILL_PATH:apm-e2e-validation}` §6.2 - `.maestro/` for Expo, `e2e/playwright/` for web, `e2e/api/` for backend - or the existing test infrastructure path if one was detected during workspace scan), the runner formats expected (Maestro YAML, Playwright spec, etc.), and the regression cadence (`per-task` default, `per-stage` for lighter overhead, `manual-only` to disable automatic regression). When the project has detected pre-existing test infrastructure, prefer integrating with it over creating a parallel directory. Omit this section when E2E policy is `never`.
   - *MCP Dependencies.* When Context Gathering identified external services requiring MCP integration, list each MCP with purpose, installation command, and authentication requirements. Resolve installation details from `{SKILL_PATH:apm-mcp-setup}` §5 Registry when the MCP is known; when not in the registry, ask the User for specifics and document them inline.
2. Read `.apm/spec.md`, then write the full Spec per §4.1 Spec Format. Set `title` to the project name, `modified` to "Spec creation by the Planner.", and fill `## Overview` with 3-5 sentences (project type, core problem, essential scope, success criteria). Let content structure follow the decisions identified. When the analysis identified an E2E Testing Policy, include an `## E2E Testing Policy` section; when policy is not `never`, also include an `## E2E Test Corpus` section per `{SKILL_PATH:apm-e2e-validation}` §6.2 and §7.1; when it identified MCP Dependencies, include an `## MCP Dependencies` section per `{SKILL_PATH:apm-mcp-setup}` §2 Declaration.
...
1. Analyze work structure from gathered context and the approved Spec per §2.2 Decomposition Principles:
   ...
   - *Stage N.* For each Stage:
     ...
     - *Per-Task analysis.* For each Task:
       ...
       - *Task validation:* concrete criteria that verify the Task's deliverables - what to check and how. Note where User involvement is needed. Validation criteria co-define the Task with Guidance. When the project's E2E Testing Policy is not `never` and the Task has observable user-facing behavior, include at least one product-level acceptance statement (user action and expected outcome) phrased implementation-agnostically - this becomes the Manager's input for scenario rendering per `{SKILL_PATH:apm-e2e-validation}` §3 Three-Level Scenario Rendering.
...
**Optional sections:** When applicable per §2.3 Spec Standards, include `## E2E Testing Policy` (declares the project's E2E policy: `auto`, `ask-per-task`, or `never`, with brief rationale), `## E2E Test Corpus` (corpus path, runner formats, and regression cadence - included when policy is not `never`), and `## MCP Dependencies` (table listing required MCPs with purpose, install command, and authentication requirements). All optional sections are omitted when the project has no end-to-end validation needs or external service integrations.
